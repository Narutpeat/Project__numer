...
import ShowBackward1 from '../component/componentnumer/showBackward1'
var Algebrite = require('algebrite'); 

export default class Backward1 extends Component {
    ...
    componentDidMount()
    {
        ...
        axios.get("http://192.168.99.100:8080/api/users/showBackward1").then(res=>{
    ...
    evaluate (){
        ...
        var result=0
        if(d==1){
            result=(f(fx,x)-f(fx,x-h))/h
        }
        else if(d==2){
            result=(f(fx,x)-2*f(fx,x-h)+f(fx,x-h-h))/Math.pow(h,2)
        }
        else if(d==3){
            result=(f(fx,x)-3*f(fx,x-h)+3*f(fx,x-h-h)-f(fx,x-h-h-h))/Math.pow(h,3)
        }
        else if(d==4){
            result=(f(fx,x)-4*f(fx,x-h)+6*f(fx,x-h-h)-4*f(fx,x-h-h-h)+f(fx,x-h-h-h-h))/Math.pow(h,4)
        }
        var TRUE=0
        if(d==1){
            TRUE=f1(fx,x)
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addBackward1',{equation:this.state.equation,x:this.state.x,h:this.state.h,d:this.state.d})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:750}}>
                    ...
                    {<ShowBackward1 users={this.state.users}/>}
                    <div id='myDiv'></div>
                    <br></br>
                    <div id='myDiv3'></div>
            </div>  
