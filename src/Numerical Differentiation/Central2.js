...
import ShowCentral2 from '../component/componentnumer/showCentral2'
var Algebrite = require('algebrite'); 

export default class Central2 extends Component {
    ...
    evaluate (){
        ...
        var result=0
        if(d==1){
            result=(-f(fx,x+h+h)+8*f(fx,x+h)-8*f(fx,x-h)+f(fx,x-h-h))/(12*h)
        }
        else if(d==2){
            result=(-f(fx,x+h+h)+16*f(fx,x+h)-30*f(fx,x)+16*f(fx,x-h)-f(fx,x-h-h))/(12*Math.pow(h,2))
        }
        else if(d==3){
            result=(-f(fx,x+h+h+h)+8*f(fx,x+h+h)-13*f(fx,x+h)+13*f(fx,x-h)-8*f(fx,x-h-h)+f(fx,x-h-h-h))/(8*Math.pow(h,3))
        }
        else if(d==4){
            result=(-f(fx,x+h+h+h)+12*f(fx,x+h+h)-39*f(fx,x+h)+56*f(fx,x)-39*f(fx,x-h)+12*f(fx,x-h-h)-f(fx,x-h-h-h))/(6*Math.pow(h,4))
        }
        var TRUE=0
        if(d==1){
            TRUE=f1(fx,x)
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addCentral2',{equation:this.state.equation,x:this.state.x,h:this.state.h,d:this.state.d})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:750}}>
                    <h3>Central Divided Difference O(h^2)</h3>
                    <div>
                    ...
                    {<ShowCentral2 users={this.state.users}/>}
                    <div id='myDiv'></div>
                    <br></br>
                    <div id='myDiv3'></div>
            </div>  
