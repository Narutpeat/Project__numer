...
import ShowCentral1 from '../component/componentnumer/showCentral1'
var Algebrite = require('algebrite'); 

export default class Central1 extends Component {
    ...
    componentDidMount()
    {
        ...
        axios.get("http://192.168.99.100:8080/api/users/showCentral1").then(res=>{
    ...
    evaluate (){
        ...
        var result=0
        if(d==1){
            result=(f(fx,x+h)-f(fx,x-h))/(2*h)
        }
        else if(d==2){
            result=(f(fx,x+h)-2*f(fx,x)+f(fx,x-h))/Math.pow(h,2)
        }
        else if(d==3){
            result=(f(fx,x+h+h)-2*f(fx,x+h)+2*f(fx,x-h)-f(fx,x-h-h))/(2*Math.pow(h,3))
        }
        else if(d==4){
            result=(f(fx,x+h+h)-4*f(fx,x+h)+6*f(fx,x)-4*f(fx,x-h)+f(fx,x-h-h))/Math.pow(h,4)
        }
        var TRUE=0
        if(d==1){
            TRUE=f1(fx,x)
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addCentral1',{equation:this.state.equation,x:this.state.x,h:this.state.h,d:this.state.d})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:750}}>
                    ...
                    {<ShowCentral1 users={this.state.users}/>}
                    <div id='myDiv'></div>
                    <br></br>
                    <div id='myDiv3'></div>
            </div>  
