...
import ShowBackward2 from '../component/componentnumer/showBackward2'
var Algebrite = require('algebrite'); 

export default class Backward2 extends Component {
    ...
    evaluate (){
        ...
        var result=0
        if(d==1){
            result=(3*f(fx,x)-4*f(fx,x-h)+f(fx,x-h-h))/(2*h)
        }
        else if(d==2){
            result=(2*f(fx,x)-5*f(fx,x-h)+4*f(fx,x-h-h)-f(fx,x-h-h-h))/Math.pow(h,2)
        }
        else if(d==3){
            result=(5*f(fx,x)-18*f(fx,x-h)+24*f(fx,x-h-h)-14*f(fx,x-h-h-h)+f(fx,x-h-h-h-h))/(2*Math.pow(h,3))
        }
        else if(d==4){
            result=(3*f(fx,x)-14*f(fx,x-h)+26*f(fx,x-h-h)-24*f(fx,x-h-h-h)+11*f(fx,x-h-h-h-h)-2*f(fx,x-h-h-h-h-h))/Math.pow(h,4)
        }
        var TRUE=0
        if(d==1){
            TRUE=f1(fx,x)
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addBackward2',{equation:this.state.equation,x:this.state.x,h:this.state.h,d:this.state.d})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:750}}>
                    <h3>Backward Divided Difference O(h^2)</h3>
                    <div>
                    ...
                    {<ShowBackward2 users={this.state.users}/>}
                    <div id='myDiv'></div>
                    <br></br>
                    <div id='myDiv3'></div>
            </div>  
