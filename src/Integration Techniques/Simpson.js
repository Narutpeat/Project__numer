...
import ShowSimpson from '../component/componentnumer/showSimpson'
...
export default class Simpson extends Component {
    ...
    evaluate (){
        ...
        var h=(b-a)/2
        var result=(h/3)*(f(fx,a)+(4*f(fx,a+h))+f(fx,b))
        var A =new Array()
        A[0]=a
        A[1]=a+h
        A[2]=b
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addSimpson',{equation:this.state.equation,a:this.state.a,b:this.state.b})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:550}}>
                    ...
                    {<ShowSimpson users={this.state.users}/>}
                    <div id='myDiv'></div>
                    <br></br>
                    <div id='myDiv3'></div>
            </div>  
