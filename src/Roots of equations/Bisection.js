...
import ShowBisection from '../component/componentnumer/showBisection'
...
export default class Bisection extends Component {
    ...
    evaluate (){
        ...
        while(i<1 || er(xn,xo)>0.000001){
            ...
            var xm=(xl[i]+xr[i])/2
            var fxm=f(fx,xm)
            if((fxm>0&&fxr[i]>0)||(fxm<0&&fxr[i]<0)){
                xo=xr[i]
                xr[i+1]=xm
                xl[i+1]=xl[i]
                xn=xm
    ...
    sendDatatoDB = ()=>{
        axios.post('http://192.168.99.100:8080/api/users/addBisection',{equation:this.state.equation,xl:this.state.xl,xr:this.state.xr})
        .then(res=>{
            console.log(res);
        })
        this.evaluate()
    ...
    render()
    {
        return(
            <div style={{width:550}}>
                    ...
                    {<ShowBisection users={this.state.users}/>}
                    { this.state.show && <Box />}
                    <div id='myDiv'></div>
...
class Box extends Component{
    render(){
        return(
            <div style={{ textAlign: 'center', width: '650%'  ,fontSize: 30 ,fontWeight: 'bold'}}>Iteration</div>
        )
    }
}
...
class Box2 extends Component{
    render(){
        return(
            <div style={{ textAlign: 'center', width: '650%'  ,fontSize: 30 ,fontWeight: 'bold'}}>Error</div>
        )
    }
}
...
class Box3 extends Component{
    render(){
        return(
            <div style={{ textAlign: 'center', width: '650%'  ,fontSize: 30 ,fontWeight: 'bold'}}>Graph</div>
        )
    }
}
